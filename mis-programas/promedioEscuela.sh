...
echo -e "\033[94;105mTexto en azul brillante con fondo magenta brillante\033[0m
"
...
echo
saludar
echo -e "\n ${yellowColour}Correcto${endColour} ${greenColour} $usuario ${endColour} ${yellowColour}Elige que materia quieres llevar, y después pon tus 4 calificaciones del semestre.\n Con base en esas calificaciones obtendrás tu promedio. ${endColour}"
echo -e "\n ${redColour}############################################################################### ${endColour}"
sleep 15
clear
#fin saludo
...
if [ $s1 == 1 ]; then
    ...
    echo "Tú elegiste $asignatur"
elif [ $s1 == 5 ]; then
    asignatura="Disciplina"
    echo "Tú elegiste $asignaturea"
else
    sleep 2
    clear
...
read -p "$signature primer bloque: " f4
read -p "$signature segundo bloque: " s4
read -p "$signature tercer bloque: " t4
read -p "$signature cuarto bloque: " f4
...
promedio=$(( p1 +p2 ))
let promedio/=2
echo "Promedio del semestre en $asignatura: $promedio"
#fin calculadora
